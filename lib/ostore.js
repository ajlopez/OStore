...
var exproperators = {
    $eq: function (data, values) { return getValue(data, values[0]) == getValue(data, values[1]); },
    $ne: function (data, values) { return getValue(data, values[0]) != getValue(data, values[1]); }
}
...
function evaluate(data, expr) {
    for (var n in expr)
        if (exproperators[n])
            return exproperators[n](data, expr[n]);
    if (expr.$lt)
        return getValue(data, expr.$lt[0]) < getValue(data, expr.$lt[1]);
    if (expr.$lte)
        return getValue(data, expr.$lte[0]) <= getValue(data, expr.$lte[1]);
    if (expr.$gt)
