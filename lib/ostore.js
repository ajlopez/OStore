...
function Store(name) {
    ...
    this.remove = function (id) {
        if (typeof id === 'object') {
            var criteria = id;
            var toremove = findValues(values, criteria);
    ...
    this.update = function (id, newvalues, options) {
        options = options || { };
        
        if (typeof id === 'object') {
            var criteria = id;
            var toupdate = findValues(values, criteria);
...
function findValues(values, criteria) {
    var result = [];
...
function sort(items, sspec) {
    var name = Object.keys(sspec)[0];
    var descending = sspec[name] !== 1 && sspec[name] !== true;
        ...
        if (n[0] === '$') {
            if (operators[n]) {
                if (typeof criterium === 'object' && !Array.isArray(criterium))
                    criterium = evaluate(value, criterium);
